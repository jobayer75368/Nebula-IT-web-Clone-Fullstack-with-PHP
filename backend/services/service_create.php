...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) {
        $sql = "INSERT INTO services (title, slug, short_description, long_description,featured_image, status)
                    VALUES (?,?,?,?,?,?)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$title, $slug, $short_description, $long_description, $featured_image, $status]);
